...
@interface WDJifenDetailViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
static NSString *lists = @"WDjifenListsCell";
@implementation WDJifenDetailViewController
...
- (void)viewDidLoad {
    ...
    _goodsListsTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_detailsView.frame)+15, kScreenWidth, 0) style:UITableViewStylePlain];
...
- (void)_loadOrderDatas{
    ...
    [WDMineManager requestOrderMsgWithOid:self.orderId completion:^(WDOrderMsgModel *model, NSString *error) {
        ...
        self.orderNumber.text = model.osn;
        self.orderTime.text = model.addtime;
        self.payWay.text = model.paysystemtype;
        self.receiveName.text = model.consignee;
        self.receivePhone.text = model.mobile;
        self.receiveAddress.text = model.address;
        self.jifenCount.text = model.creditsvalue;
